...
func _ready() -> void:
	...
	EngineDebugger.register_message_capture(&"error", _on_error_message)
...
func _exit_tree() -> void:
	...
	if EngineDebugger.is_active():
		...
		EngineDebugger.unregister_message_capture(&"error")
...
func _dispatch_tool(request_id: String, tool_name: String, args_json: String) -> void:
	...
	# Async: explore_camera capture needs await, intercept before synchronous _execute
	if tool_name == &"explore_camera":
		var action: Variant = args.get(&"action", args.get(&"properties", {}).get(&"action"))
		if action == &"capture":
			_explore_camera_capture_async(request_id)
			return
	if tool_name == &"inject_input" and args.has(&"track"):
...
func _execute(tool_name: String, args: Dictionary) -> Dictionary:
	match tool_name:
		...
		&"map_ui":
			return _map_ui(args)
		&"explore_camera":
			return _dispatch_explore_camera(args)
		&"runtime_nav":
...
# =============================================================================
# Explore camera (Feature 3)
# =============================================================================
func _dispatch_explore_camera(args: Dictionary) -> Dictionary:
	args.merge(args.get(&"properties", {}))
	match args[&"action"]:
		&"spawn":
		...
		&"capture":
			return {&"err": "capture requires async dispatch — should not reach here"}
		&"restore":
			_restore_explore_camera()
			return {}
	return {&"err": "Unknown action (use spawn, move, capture, restore)"}
...
# =============================================================================
func _on_error_message(message: String, data: Array) -> bool:
	...
	match message:
		&"error", &"script_error":
			level = "error"
	_log_buffer.append({&"msg": data[0] if data.size() > 0 else message, &"level": level, &"timestamp": Time.get_ticks_msec()})
	return false # don't consume — let debugger process normally
